...
// FTCoin note: this is only useful during the initial proof-of-work 'IPO' phase of the network
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("numberoftransactions",   (int)pwalletMain->mapWallet.size()));
    ...
    obj.push_back(Pair("iscrypted",      pwalletMain->IsCrypted()));
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new FTCoin address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current FTCoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <FTCoinaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid FTCoin address");
...
Value edittaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "edittaccount <FTCoinaddress> <account>\n"
            "Sets the account associated with the given address.");
...
Value getpeercoinaddresses(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getbitcoinaddresses <account>\n"
            "Returns the list of addresses and the associated Bitcoin address for the given account.");
...
Value settxfee(const Array& params, bool fHelp)
{
    ...
    nTransactionFee = (nTransactionFee / uCOIN) * uCOIN;  // round to cent
    return true;
}
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 4))
        throw runtime_error(
            "sendtoaddress <FTCoinaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <FTCoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <FTCoinaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <FTCoinaddress> [minconf=1]\n"
            "Returns the total amount received by <FTCoinaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
            "sendfrom <fromaccount> <toftcoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "requires portfolio passphrase to be set with walletpassphrase first");
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() < 3 || params.size() > 6))
        throw runtime_error(
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid FTCoin address:")+s.name_);
...
Value distribute(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        {
            Object obj;
            obj.push_back(Pair("ftcoin_address", distribution.GetPeershareAddress().ToString()));
            obj.push_back(Pair("balance", (double)distribution.GetBalance() / COIN));
            obj.push_back(Pair("bitcoin_address", distribution.GetPeercoinAddress().ToString()));
...
Value listaddresses(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
                "listaddresses\n"
                "Returns an array of objects containing:\n"
                "  \"address\" : address in the address book\n"
                "  \"account\" : the account of the address\n"
                "  \"dividend address\" : the dividend address\n"
                "  \"ismine\" : whether the address belong to me\n");
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CBitcoinAddress& address = item.first;
        ...
        obj.push_back(Pair("address",       address.ToString()));
        obj.push_back(Pair("account",       strAccount));
        obj.push_back(Pair("dividend address", peercoinAddress.ToString()));
        obj.push_back(Pair("ismine", pwalletMain->HaveKey(address)));
        ret.push_back(obj);
    }
    return ret;
}
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Portfolio encrypted; FTCoin server stopping. Please restart server to run with encrypted portfolio";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <ftcoinaddress>\n"
            "Return information about <ftcoinaddress>.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "FTCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "FTCoin is downloading blocks...");
...
Value reservebalance(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        if (fReserve)
        {
            ...
            int64 nAmount = AmountFromValue(params[1]);
            nAmount = (nAmount / uCOIN) * uCOIN;  // round to cent
            if (nAmount < 0)
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "listaddresses",          &listaddresses,          true },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: ftcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer2(void* parg)
{
    ...
    if (mapArgs["-rpcpassword"] == "")
    {
        ...
        string strWhatAmI = "To use ftcoind";
...
std::string CallPeercoinRPC(const std::string &strMethod, const Array &params)
{
    ...
    if (!d.connect(GetPeercoinArg("-rpcconnect", "127.0.0.1"), GetPeercoinArg("-rpcport", CBigNum(fTestNet? PEERCOIN_TESTNET_RPC_PORT : PEERCOIN_RPC_PORT).ToString().c_str())))
        throw runtime_error("couldn't connect to Bitcoin RPC server");
